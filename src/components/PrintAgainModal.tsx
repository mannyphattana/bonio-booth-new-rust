...
export default function PrintAgainModal({
  ...
  useEffect(() => {
    if (timeLeft === 0 && step !== "success" && !paidRef.current) {
      appLogger.info(CTX, "Modal expired (5 min) — closing without payment");
      onCloseRef.current();
    }
  }, [timeLeft, step]);
  ...
  const finishPaid = useCallback(
    (txId: string) => {
      ...
      setStep("success");
      appLogger.info(CTX, "Reprint payment success — txId:", txId, "qty:", quantity);
      setTimeout(() => {
  ...
  const startQrPayment = useCallback(async () => {
    ...
    try {
      ...
      appLogger.error(CTX, "startQrPayment failed:", err);
  ...
  const submitCoupon = useCallback(async () => {
    ...
    try {
      ...
      appLogger.error(CTX, "submitCoupon failed:", err);
      setError("คูปองไม่สามารถใช้งานได้");
    } finally {
      setCouponLoading(false);
    }
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      ...
      try {
        ...
        appLogger.error(CTX, "check_payment_status error:", err);
      } finally {
