...
import type { ThemeData, MachineData, Capture, FrameSlot, FrameData } from "../App";
...
export default function SlotSelection({ theme }: Props) {
  ...
  const [photoAssignments, setPhotoAssignments] = useState<{ [slotIndex: number]: number }>({});
  ...
  const calculateScaleFactor = useCallback(() => {
    ...
    setScaleFactor({ x: renderedWidth / frameWidth, y: renderedHeight / frameHeight });
  ...
  const handlePhotoClick = useCallback((idx: number) => {
    ...
    if (selectedPhotos.includes(idx)) {
      ...
      setSelectedPhotos(newPhotos);
      setPhotoAssignments({ ...photoAssignments, [selectedPhotos.length]: idx });
    }
  ...
  return (
    <div className="page-container" style={{
      ...
      <header style={{
        ...
        padding: "0 40px", zIndex: 100, position: "relative"
      }}>
        <div style={{ zIndex: 110 }}>
          <BackButton onBackClick={() => navigate("/main-shooting", { state })} />
        </div>

        {/* ปรับแก้ "เลือกรูปของคุณ" ให้อยู่บรรทัดเดียวและต่ำลง */}
        <div style={{ position: "absolute", left: "50%", transform: "translateX(-50%)", textAlign: "center", top: "50px", width: "100%" }}>
          <h1 style={{ color: "#e94560", fontSize: "3.2rem", fontWeight: "bold", margin: 0, whiteSpace: "nowrap" }}>
            เลือกรูปของคุณ
          </h1>
          <p style={{ color: theme.fontColor, opacity: 0.8, fontSize: "1.1rem", marginTop: "4px", textTransform: "uppercase", letterSpacing: "1px" }}>
      ...
      <div style={{ 
        ...
        gap: "40px", paddingBottom: "130px" // เว้นพื้นที่ให้ปุ่ม Next ด้านล่างมากขึ้น
      }}>
        
        {/* Frame */}
        <div ref={containerRef} style={{
          position: "relative", width: "85%", height: "35vh",
          aspectRatio: `${frameAspectRatio}`, isolation: "isolate"
        }}>
          <img ref={frameImgRef} src={selectedFrame.imageUrl} alt="" onLoad={calculateScaleFactor} style={{
            ...
            return (
              <div key={i} style={{
                ...
                {captureIdx !== undefined && (
                  <img src={captures[captureIdx].photo} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                )}
              </div>
            );
          })}
        ...
        {/* 3. Thumbnails */}
        <div style={{ 
          display: "flex", flexWrap: "wrap", maxWidth: "420px", 
          gap: "12px", justifyContent: "center", zIndex: 20 
        }}>
          {captures.map((cap, idx) => {
            const isSelected = selectedPhotos.includes(idx);
            const slotIdx = Object.keys(photoAssignments).find(key => photoAssignments[parseInt(key)] === idx);
            return (
              <div key={idx} onClick={() => handlePhotoClick(idx)} style={{
                width: "85px", height: "85px", borderRadius: "15px", overflow: "hidden",
                border: isSelected ? "3px solid #fff" : "3px solid transparent",
                backgroundColor: "black", cursor: "pointer", transition: "0.2s",
                position: "relative", opacity: isSelected ? 0.6 : 1,
                boxShadow: "0 4px 10px rgba(0,0,0,0.2)"
              }}>
                <img src={cap.photo} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                {isSelected && (
                  <div style={{
                    position: "absolute", top: 5, right: 5, width: "22px", height: "22px",
      ...
      <footer style={{
        ...
        <button onClick={handleNext}
          ...
          style={{
            background: selectedPhotos.length >= slots.length ? "rgba(255, 255, 255, 0.9)" : "rgba(255, 255, 255, 0.4)",
            backdropFilter: "blur(10px)", color: selectedPhotos.length >= slots.length ? "#e94560" : "#fff",
            padding: "14px 80px", // ลดขนาดปุ่มลง
            borderRadius: "40px", fontSize: "20px", fontWeight: "bold", border: "none",
            cursor: "pointer", transition: "0.3s", boxShadow: "0 10px 30px rgba(0,0,0,0.1)"
          }}>
          Next
        </button>
      </footer>
      ...
      <div style={{ position: "absolute", bottom: "30px", right: "40px", opacity: 0.8 }}>
         <span style={{ fontSize: "24px", fontWeight: "bold", color: "#e94560" }}>timelab</span>
         <span style={{ fontSize: "10px", display: "block", textAlign: "right", color: "#e94560" }}>PHOTO BOOTH</span>
      </div>
    </div>
  );
}